...
import { UploadDeliverableForm } from "@/components/admin/UploadDeliverableForm";
...
export default async function AdminOrderDetailPage({
  ...
  const { data: order } = await supabase
    .from("orders")
    .select("id, user_id, status, notes, price, created_at, services(title)")
    .eq("id", id)
    .single<OrderDetail>();
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="space-y-4">
        ...
        {order.notes && (
          <div>
            <h2 className="text-sm font-bold text-brand-charcoal">توضیحات کاربر</h2>
      ...
      <Card className="space-y-4">
        <h2 className="text-base font-bold text-brand-charcoal">فایل‌های ورودی کاربر</h2>
        ...
        {inputFiles.length > 0 ? (
          ...
          <p className="text-sm leading-7 text-brand-charcoal/70">
            کاربر هنوز فایلی آپلود نکرده است.
          </p>
        )}
      </Card>

      <Card className="space-y-4">
        <h2 className="text-base font-bold text-brand-charcoal">فایل‌های گزارش نهایی</h2>
